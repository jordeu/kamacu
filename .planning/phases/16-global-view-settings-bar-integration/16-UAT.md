...
number: 1
name: Live bar row + click-through + highlight (GINT-01)
expected: |
  A live global agent session renders as a Global · Scratchpad bar row; clicking navigates to /global; the row highlights while /global is the current page; task rows keep their existing navigation/highlight
