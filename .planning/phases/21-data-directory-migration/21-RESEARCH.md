...
3. **Live E2E must not risk the real 5.5 GB install.** (STATE.md calls for a real-install verification.) — **RESOLVED:** copied-HOME UAT; adopted by 21-05.
   ...
   - **CORRECTION (Gap 2, 21-07) — HOME override does NOT isolate `git`.** The recipe above rewrote the copy's DB paths but NOT its on-disk git link files, which bake the *absolute* real prefix `/home/jordi/.kangent`. After the rename, `git worktree repair` followed those absolute pointers **into the real install**. `git` resolves `gitdir`/`.git` link files by absolute path regardless of `$HOME`, so a copy that still carries the real prefix reaches back into the live data. A safe UAT MUST, **before any boot**, rewrite the real-home prefix (`$HOME/.kangent` → `$TMPHOME/.kangent`) in BOTH:
     - **the DB** (`projects.repo_path` for managed repos, `tasks.worktree_path`, and any raw `worktree_base` setting) — with a `VACUUM` afterward so no stale prefix lingers in freelist pages; and
     - **all on-disk git link files** — every linked worktree's `.git` gitfile (`gitdir: <abs>`) plus every `<repo>/.git/worktrees/*/gitdir` (also check `commondir` and `config`) that bakes the real prefix.
   - **ISOLATION GATE (pre-boot, MUST pass before booting):** assert `grep -rl "$HOME/.kangent" "$TMPHOME"` returns **zero files** — no surviving real-home reference anywhere in the copy. Use **REAL GNU `grep`** here, not a shell-aliased/wrapped `grep` (e.g. one routed to `ugrep`/`rg`) — a wrapped grep can silently skip `.git` directories or dot-dirs and give a false-clean gate that lets the boot reach into the real install. After this rewrite the copy is fully self-contained: post-rename the git pointers dangle at `$TMPHOME/.kangent/...` exactly like a real in-place upgrade, and `git worktree repair` fixes them WITHIN the copy.
