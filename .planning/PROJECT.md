...
- [~] Rename kangent → kamacu: code identity + UI/brand done (Phase 20); gated `~/.kangent` → `~/.kamacu` data migration (git worktree repair, DB path rewrite, tmux socket/session prefix, localStorage keys) remains — Phase 21
- [x] Kamacu logo + favicon; README.md — Phase 20
- [x] GitHub "Files changed"-style diff view (file tree, collapsible diffs, persistent per-file "Viewed" with change-reset) — Phase 22
...
---
*Last updated: 2026-07-02 — v1.8 Phase 22 (GitHub-Style Diff Review) complete: two-pane Diff tab (file tree + scroll-spy, stacking sticky headers, keep-history per-file Viewed with change-reset); code-review BLOCKER (dash-prefixed untracked path aborting the diff) + two warnings fixed*
